...
def run_step(models, data, run_type='train'):
	...
	shape = source_aligned.shape

	n = shape[1]//3
	H, W = shape[-2], shape[-1]

	perturbed = torch.vstack([
		perturbed[i:i+n].view(-1, H, W).unsqueeze(0)
			for i in range(len(perturbed) - n + 1)])
...
def validation(models, val_loader, device, epoch):
	...
	for val_i, data in enumerate(tqdm(val_loader)):
		...
		sample = transform_video_dimension(sample, H, W)
