...
from dataset import LMDBDataset, NPZDataset
...
def train(args, epoch, loader, model, optimizer, scheduler, device):
    ...
    checkpoint_dir = 'checkpoint_sched' if args.sched is not None else 'checkpoint'

    if args.hier == 'bottom':
        checkpoint_dir = f'{checkpoint_dir}_bottom'
    ...
    for i, (top, bottom, label) in enumerate(loader):
        ...
        if args.hier == 'top':
            ...
            bottom = bottom.squeeze(0).to(device)
            target = bottom
...
if __name__ == '__main__':
    ...
    dataset = NPZDataset(args.path, args.batch)
    loader = DataLoader(
