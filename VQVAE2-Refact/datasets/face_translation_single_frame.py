...
class FacialTransformsSingleFrameDataset(Dataset):
	def __init__(self, mode='train', add_perturbation=True):
		self.mode = mode
	...
	def getitem(self, idx):
		...
		if self.perturbation: 
			...
			raw_image_resized = resize_frame(raw_image)
			convex_mask = generate_convex_hull(raw_image_resized, landmark[17:])
			face_segmented = apply_mask(convex_mask, raw_image_resized)

			face_segmented_perturbed = perturbations.perturb_image(face_segmented)

			combined_image = combine_images(raw_image_resized, face_segmented_perturbed)

			raw_image = combined_image.copy()

		transformed_image = transforms.Compose(self.transform)(raw_image)

		self.transformElements[1] = transforms.Pad((padw, padh))
		ground_truth = transforms.Compose(self.transformElements)(raw_image_resized)
		# print(transformed_image.shape)
		return transformed_image, ground_truth
