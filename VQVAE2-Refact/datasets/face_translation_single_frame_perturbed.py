...
class FacialTransformsSingleFrameDatasetPerturbed(Dataset):
	def __init__(self, mode='train', add_perturbation=True, require_color_jitter=True):
		self.mode = mode

		self.H, self.W = 256, 256

		self.perturbation = add_perturbation
		self.color_jitter = require_color_jitter
		...
		# This would be the image path - the perturbations have to be introduced in the same image
		# self.base = '/ssd_scratch/cvit/bipasha31/processed_image_frames/*/*'
		self.base = '/ssd_scratch/cvit/aditya1/processed_vlog_dataset_copy/*/*/*/*/*.jpg'
		...
		train, test = train_test_split(datapoints, train_size=train_split, random_state=42)
		self.images = train if mode == 'train' else test
	...
	def getitem(self, idx):
		...
		landmark_path = image_path.split('.')[0] + '_landmarks.npz'
		...
		resized_image[mask] = 0

		background_image_tensor = transforms.ToTensor()(resized_image)

		if self.color_jitter:
			face_segmented_color_jittered = transforms.Compose(self.colorJitterTransformation)(face_segmented_perturbed)
		else:
			face_segmented_color_jittered = transforms.ToTensor()(face_segmented_perturbed)

		combined_image = face_segmented_color_jittered + background_image_tensor
		combined_image = transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))(combined_image)        

		self.transformElements[1] = transforms.Pad((padw, padh))

		ground_truth_tensor = transforms.Compose(self.transformElements)(resized_image_copy)

		return combined_image, ground_truth_tensor
