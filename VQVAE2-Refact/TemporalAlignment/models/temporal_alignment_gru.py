...
class TemporalAlignment(nn.Module):
	def __init__(self):
		...
		self.post_conv = nn.Sequential(
            Conv3d(3, 3, kernel_size=5, stride=1, padding=2, residual=True),
            Conv3d(3, 3, kernel_size=5, stride=1, padding=2, residual=True),
            Conv3d(3, 3, kernel_size=3, stride=1, padding=1, residual=True),
            Conv3d(3, 3, kernel_size=3, stride=1, padding=1, residual=True),
            )

		self.conv2 = nn.Conv3d(3, 3, kernel_size=1, stride=1, padding=0)
	...
	def forward(self, source, target):
		...
		added = (source_new + target).permute(0, 2, 1, 3, 4)

		prediction = self.post_conv(added)

		out = self.conv2(added + prediction).permute(0, 2, 1, 3, 4)
