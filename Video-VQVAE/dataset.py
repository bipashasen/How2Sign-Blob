...
class HandGesturesDataset(Dataset):
    def __init__(self, path, mode, n_embed=512):
        self.embed = n_embed
        ...
        with open(os.path.join(path, '{}.txt'.format(mode))) as r:
            self.data = [os.path.join(path, x) for x in r.read().splitlines()]
    ...
    def get_dataitem(self, idx):
        ...
        if len(frames) > self.max_seq_len:
            ...
            frames = frames + [torch.zeros_like(frames[0])]*(self.max_seq_len - len(frames))
