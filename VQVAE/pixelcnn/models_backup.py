...
class GatedMaskedConv2d(nn.Module):
    ...
    def forward(self, x_v, x_h, h):
        ...
        h = self.class_cond_embedding(h) # B -> B x 1250
...
class GatedPixelCNN(nn.Module):
    ...
    def forward(self, x, label):
        # print(f'inside x: {x.shape}, label: {label.shape}')
        print(x)
        shp = x.size() + (-1, )
        ...
        print(f'inside out.shape: {out.shape}')
        assert False
    ...
    def generate(self, label, shape=(8, 8), batch_size=64):
        ...
        print(f'created random prior: {x.shape}')
